...
Também foram repetidos os testes unitários e de integração, Clippy, build Linux release, check do target Windows, smoke test headless sob Xvfb e verificação do formato PE32+. O branch continua sem alterações acidentais fora do escopo auditado. A única limitação relevante remanescente é a validação visual e funcional em Windows real, especialmente DPI, clipboard, drag and drop, diálogos e acessibilidade nativa.
